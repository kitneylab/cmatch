...
def reconstruct(matches):
    ...
    for target in targets:
        ...
        for i in range(1, len(libs), 1):
            ...
            for p in np:
                print("\tCandidate path:", p)
                EPSILON = 2
                if p[i - 1]["end"] - EPSILON <= p[i]["start"]:
                    print("\tADDDDING:", p)
                    paths.append(p)
        ...
        for path in paths:
            nn.append(path[::-1])
        print("Paths:", nn)
        scores = compute_scores(nn)
        names = construct_names(nn)
        r = []
        for i in range(len(paths)):
            d = {
                "slice_name": target["target"],
                "slice": target["slice"],
                "reconstruct": names[i],
                "score": scores[i],
                "path": nn[i],
...
def main():
    ...
    RES = "20210812-124853-matching-results-run_cm2_2-cm2-lycopene-100targets-sanger-th09-run-1-from-1.json"
    RES = "20210812-233055-matching-results-run_cm2_2-cm2-lycopene-3targets-sanger-th01-run-1-from-1.json"
    MATCHES = path.join(RES_DIR, RES)
